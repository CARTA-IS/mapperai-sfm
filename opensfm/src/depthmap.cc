...
class DepthmapEstimator {
 public:
  ...
  void AddView(const double *pK,
    ...
    betas_.emplace_back(cv::Mat(height, width, CV_64FC2));      //set to neighbors size.
    alphas_.emplace_back(cv::Mat(height, width, CV_64FC2));
    qMats_.emplace_back(cv::Mat(height, width, CV_64F));
    int a = (size > 1) ? 1 : 0;
  ...
  void ComputePatchMatch(cv::Mat *best_depth, cv::Mat *best_plane, cv::Mat *best_score, cv::Mat *best_nghbr) {
    ...
    // compute beta
    int hpz = (patch_size_-1)/2;
    for(int i = 0; i < best_plane->rows; i++){
        for(int j = best_plane->cols-hpz-1; j >= hpz; j--){
            cv::Vec3f plane = best_plane->at<cv::Vec3f>(i, j);
            ComputeAlphaBeta(i, j, plane, 1);
        }
    }
    
    for (int i = 0; i < patchmatch_iterations_; ++i) {
      PatchMatchForwardPass(best_depth, best_plane, best_score, best_nghbr, false, i%2);
      PatchMatchBackwardPass(best_depth, best_plane, best_score, best_nghbr, false, i%2);
    }
  }
  ...
  void ComputePatchMatchSample(cv::Mat *best_depth, cv::Mat *best_plane, cv::Mat *best_score, cv::Mat *best_nghbr) {
    ...
    for (int i = 0; i < patchmatch_iterations_; ++i) {
      PatchMatchForwardPass(best_depth, best_plane, best_score, best_nghbr, true, i%2);
      PatchMatchBackwardPass(best_depth, best_plane, best_score, best_nghbr, true, i%2);
    }
  }
...
//select optimized depth with bayesian
  void CheckPlaneCandidateAlpha(cv::Mat *best_depth, cv::Mat *best_plane, cv::Mat *best_score, cv::Mat *best_nghbr, 
          int i, int j, const cv::Vec3f &plane){
      float score;
...
/////generate beta
  ...
  void ComputeDepthScore(int i, int j, const cv::Vec3f &plane,float *score){
      *score = -1.0f;
      ...
      for(int other = 1; other < images_.size(); ++other){
          ...
          image_score += (1-ncc_score)*(1-ncc_score)*qMats_.at(other)[i][j];
      }
      if (image_score > *score){
          *score = image_score;
      }
 ...
 private:
  std::vector<cv::Mat> qMats_;
  std::vector<cv::Mat> betas_;
  std::vector<cv::Mat> alphas_;
